...
/*tu moŜecie dostac dodatkowe zadanie polegające na tym ze np. dla procesu o pid = 15 naleŜy zwracac zawsze 999 –robimy to tak: odkomentuj ponizsza linijke:
if(pid==15) return 999;
*/
if( ( (mproc[procNum].mp_flags & IN_USE) != 0) && (mproc[procNum].mp_pid == pid) ) return procNum;
}
return ENOENT;
}
